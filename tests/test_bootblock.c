...
static int test_valid(void)
{
    ...
    if (!report.has_dos_header || !report.checksum_valid)
        return 1;
    return report.score != 0 || report.finding_count != 0UL;
}
...
static int test_invalid(void)
{
    ...
    if (report.has_dos_header || report.checksum_valid)
        return 1;
    return report.score != 35 || report.finding_count != 2UL;
}
...
static int test_code_heuristics(void)
{
    ...
    if (!has_finding(&report, "BOOT.CODE_PRESENT") ||
        !has_finding(&report, "BOOT.BRANCH_OPCODE") ||
        !has_finding(&report, "BOOT.EXECBASE_REFERENCE") ||
        !has_finding(&report, "BOOT.CUSTOM_CHIP_REFERENCE"))
        return 1;
...
static int test_lvo_heuristic(void)
{
    ...
    if (!has_finding(&report, "BOOT.JSR_OPCODE") ||
        !has_finding(&report, "BOOT.A6_LVO_CALL") ||
        !has_finding(&report, "BOOT.RTS_OPCODE"))
        return 1;
    if (has_finding(&report, "BOOT.EXEC_MUTATION_LVO"))
        return 1;
    return report.score != 15;
}
...
static int test_exec_mutation_lvo(void)
{
    ...
    if (amiheur_boot_analyze(block, sizeof(block), &report) != 0)
        return 1;
    if (!has_finding(&report, "BOOT.EXEC_MUTATION_LVO") ||
        has_finding(&report, "BOOT.CORR_MUTATION_CONTROL"))
        return 1;
    return report.score != 35;
}
...
static int test_exec_control_lvo(void)
{
    ...
    if (!has_finding(&report, "BOOT.EXEC_CONTROL_LVO") ||
        has_finding(&report, "BOOT.EXEC_MUTATION_LVO"))
        return 1;
    return report.score != 20;
}
...
static int test_correlated_mutation(void)
{
    ...
    make_dos_block(block);
    put_jsr_a6(block + 12U, -420);
    put_jsr_a6(block + 16U, -132);
    write_be32(block + 20U, 4UL);
    write_be32(block + 24U, 0x00dff180UL);
    block[28] = 0x12U;
    block[29] = 0x34U;
    block[30] = 0x56U;
    block[31] = 0x78U;
    ...
    if (amiheur_boot_analyze(block, sizeof(block), &report) != 0)
        return 1;
    if (!has_finding(&report, "BOOT.CORR_MUTATION_CONTROL"))
        return 1;
    if (!has_finding(&report, "BOOT.CORR_MUTATION_HARDWARE"))
        return 1;
    if (!has_finding(&report, "BOOT.CORR_MUTATION_EXECBASE"))
        return 1;
    if (report.score < 80)
        return 1;
    return 0;
...
int main(void)
{
    ...
    if (test_correlated_mutation() != 0) {
        puts("FAIL: correlated mutation heuristic");
        return 1;
    }
    puts("PASS: bootblock validation, Exec semantics, and correlation");
    return 0;
}
