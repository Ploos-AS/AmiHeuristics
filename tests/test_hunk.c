...
static int test_valid(void)
{
    ...
    if (amiheur_hunk_analyze(b, n, &r) != 0)
        return 1;
    if (!r.valid || r.hunk_count != 1UL || r.code_bytes != 8UL)
        return 1;
    if (!r.has_code || !r.has_a6_lvo_call || !r.has_execbase_a6_load)
        return 1;
    if (!finding(&r, "HUNK.A6_LVO_CALL") ||
        !finding(&r, "HUNK.EXECBASE_A6_LOAD"))
        return 1;
...
static int test_control_mutation(void)
{
    ...
    if (amiheur_hunk_analyze(b, n, &r) != 0 || !r.valid)
        return 1;
    if (!r.has_execbase_a6_load || !r.has_exec_control_lvo ||
        !r.has_exec_mutation_lvo)
        return 1;
...
static int test_unproven_a6(void)
{
    unsigned char b[128];
    AmiHeurHunkReport r;
    size_t n;
    n = make_unproven_a6(b);
    if (amiheur_hunk_analyze(b, n, &r) != 0 || !r.valid)
        return 1;
    if (!r.has_a6_lvo_call || r.has_execbase_a6_load)
        return 1;
